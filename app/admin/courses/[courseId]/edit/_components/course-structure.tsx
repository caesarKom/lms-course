...
export const CourseStructure = ({ data }: Props) => {
  const initialItems =
    data.chapter.map((chapter) => ({
      ...
      isOpen: true,
      lessons: chapter.lesson.map((les) => ({
        id: les.id,
        title: les.title,
        order: les.position,
      })),
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if (activeType === "chapter") {
      ...
      if (!targetChapterId) {
        toast.error("Could not datermine the chapter for reordering");
        return;
      }
    ...
    if (activeType === "lesson" && overType === "lesson") {
      ...
      if (oldLessonIndex === -1 || newLessonIndex === -1) {
        toast.error("Could not found lesson for reordering");
        return;
      }
